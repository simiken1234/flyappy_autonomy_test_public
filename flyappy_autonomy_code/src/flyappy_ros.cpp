...
void FlyappyRos::gameEndedCallback(const std_msgs::Int32::ConstPtr& msg) // Used to be Bool
{
    /* if (msg->data)
    {
    ...
    scores_.push_back(msg->data);
    int max_score = 0;
    // Print all scores
    for (int i = 0; i < scores_.size(); i++)
    {
        ROS_INFO("Score %d: %f", i, scores_[i]);
        if (scores_[i] > max_score)
        {
            max_score = scores_[i];
        }
    }
    ROS_INFO("Max score: %d", max_score);
